...
// Chizmaning chap yuqorisidagi belgi — amalning o‘zi (fotoapparat,
// qalam). Ilovadagi kartochka belgisi bilan AYNAN bir xil bo‘lsin
// uchun ikona ro‘yxatidan olinadi (ega talabi, 2026-08-31).
function gdBadge(name) {
  const p = ICON_PATHS[name] || "";
  const line = Array.isArray(p) ? p[1] : p;
  return '<circle class="gd-badge-ring" cx="44" cy="28" r="21.5"/>' +
    '<circle class="gd-brand-fill" cx="44" cy="28" r="18"/>' +
    '<g class="gd-badge-ic" transform="translate(34 18) scale(.834)">' + line + '</g>';
}
...
const GUIDE_ART = {
  ...
  "page-photo":
    GD_BOOK + GD_PAGENUM + gdBadge("camera") +
    gdPhone(
  ...
  "page-manual":
    GD_BOOK + GD_PAGENUM + gdBadge("edit") +
